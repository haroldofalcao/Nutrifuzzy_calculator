...
def calcular_submodulo_fenotipico(imc_valor, perda_valor, sarcopenia_valor):
    ...
    regras = [
        ...
        ctrl.Rule(imc['alto_risco'] & perda_ponderal['baixo_risco'], risco_fenotipico['baixo_moderado']),
        ctrl.Rule(imc['medio_risco'] & perda_ponderal['alto_risco'] & sarcopenia['alto_risco'], risco_fenotipico['moderado_alto']),
        ctrl.Rule(imc['medio_risco'] & perda_ponderal['alto_risco'] & sarcopenia['medio_risco'], risco_fenotipico['moderado']),
...
def calcular_submodulo_ingestao(vet_valor, duracao_valor, sintomas_valor):
    ...
    Total: 19 regras fuzzy (15 originais + 4 de cobertura)
    
    CORREÇÃO v1.1: Adicionadas 4 regras de cobertura para evitar erros de defuzzificação
    em combinações específicas como VET=medio + duracao=baixo + sintomas=alto
    """
    ...
    # 15 regras originais + 4 regras de cobertura = 19 regras
    regras = [
        ...
        ctrl.Rule(vet_consumido['medio_risco'] & duracao_deficit['alto_risco'] & sintomas_gi['alto_risco'], risco_ingestao['moderado_alto']),
...
def calcular_submodulo_inflamatorio(pcr_valor, albumina_valor, febre_valor):
    ...
    regras = [
        ...
        ctrl.Rule(pcr['alto_risco'] & albumina['alto_risco'], risco_inflamatorio['alto']),
        ctrl.Rule(pcr['alto_risco'] & albumina['baixo_risco'] & febre['medio_risco'], risco_inflamatorio['moderado_alto']),
        ctrl.Rule(pcr['medio_risco'] & albumina['alto_risco'] & febre['alto_risco'], risco_inflamatorio['moderado_alto']),
...
def calcular_risco_final_integrado(escore_fen, escore_ing, escore_inf, escore_grav):
    ...
    regras = [
        ...
        ctrl.Rule(escore_ingestao['alto'] & escore_gravidade_int['moderado_alto'], risco_final['moderado_alto']),
        ctrl.Rule(escore_fenotipico['alto'] | escore_ingestao['alto'] | escore_gravidade_int['alto'], risco_final['moderado_alto']),
        ctrl.Rule(escore_fenotipico['moderado'] & escore_gravidade_int['moderado'], risco_final['moderado']),
        ctrl.Rule(escore_ingestao['moderado'] & escore_inflamatorio['moderado'], risco_final['moderado']),
        ...
        ctrl.Rule(escore_inflamatorio['alto'] & escore_gravidade_int['alto'], risco_final['moderado_alto'])
    ]
